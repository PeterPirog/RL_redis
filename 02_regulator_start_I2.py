...
class PolicyPi():
    def __init__(self,regulator_interface,random=True):
        ...
        self.action_continous=self.reg_i.action_continous
        self.n_actions=self.reg_i.n_actions
        self.random=random

    def __call__(self, observation):
        if self.random:  #for random regulator
            if self.action_continous:  #for continous environments
...
if __name__=='__main__':
    ...
    episode=0
    while not reg_i.stop_gathering:  #when database has flag dor collect
        episode_rewards=0
        obs=env.reset()
